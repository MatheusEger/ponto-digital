...
import { getAdminConfig, listEmployees, invalidateCaches } from '@/lib/queries';
...
export async function POST(req: Request) {
  ...
  if (cfg.last_report_sent_at) {
    const last = cfg.last_report_sent_at;
    if (last.startsWith(today)) {
      return jsonOk({ skipped: true, reason: 'ALREADY_SENT_TODAY', last });
    }
  }
  ...
  if (records.length === 0) {
    return jsonOk({ skipped: true, reason: 'NO_RECORDS_TODAY' });
  }
  ...
  try {
    ...
    return jsonError('EMAIL_FAILED', `Falha ao enviar: ${err instanceof Error ? err.message : 'erro'}`, 500);
  }
  ...
  await enqueueWrite(async () => {
    await db.execute({
      sql: "UPDATE admin_config SET last_report_sent_at = datetime('now'), updated_at = datetime('now') WHERE id = 'singleton'",
      args: []
    });
    invalidateCaches();
  });

  return jsonOk({ sent: true, employeesWithRecords: rows.length });
}
